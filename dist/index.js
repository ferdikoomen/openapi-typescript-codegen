 ...
 * @param request Path to custom request file
 */
const writeClient = async (client, templates, output, httpClient, useOptions, useUnionTypes, exportCore, exportServices, exportModels, exportClient, exportIndex, exportSchemas, indent, postfixServices, postfixModels, clientName, request) => {
...
const writeClientClassCustomTemplate = async (client, outputPath, httpClient, useOptions, useUnionTypes, indent, postfix, templatePath, clientName) => {
    ...
    const clientClassTemplate = Handlebars.compile(await fsExtra.readFile(templatePath, 'utf8'));
    const clientClassDir = path.resolve(outputPath, 'clientClass');
    await fsExtra.remove(clientClassDir);
    await fsExtra.mkdir(clientClassDir);
...
const writeClientIndexCustomTemplate = async (client, outputPath, httpClient, useOptions, useUnionTypes, indent, postfixServices, postfixModels, templatePath, exportCore, exportServices, exportModels, exportSchemas, exportClient, clientName) => {
    registerHandlebarTemplates({
        httpClient,
        useUnionTypes,
        useOptions,
    ...
    const indexTemplate = Handlebars.compile(await fsExtra.readFile(templatePath, 'utf8'));
    ...
    await writeFile(path.resolve(outputPath, 'index.ts'), formatIndentation(formatCode(templateResult), indent));
};

const writeClientServicesCustomTemplate = async (client, outputPath, httpClient, useOptions, useUnionTypes, indent, postfix, templatePath, exportClient, clientName) => {
    registerHandlebarTemplates({
        httpClient,
        useUnionTypes,
        useOptions,
 ...
 */
const generate = async ({ input, output, httpClient = exports.HttpClient.FETCH, clientName, useOptions = false, useUnionTypes = false, exportCore = true, exportServices = true, exportModels = true, exportClient = true, exportIndex = true, exportSchemas = false, indent = exports.Indent.SPACE_4, postfixServices = 'Service', postfixModels = '', request, serviceTemplate, clientTemplate, indexTemplate, write = true, }) => {
    ...
    switch (openApiVersion) {
        case OpenApiVersion.V2: {
            ...
            if (!write)
                break;
            await writeClient(clientFinal, templates, output, httpClient, useOptions, useUnionTypes, exportCore, exportServices, exportModels, exportClient, exportIndex, exportSchemas, indent, postfixServices, postfixModels, clientName, request);
            break;
        }
    ...
    if (serviceTemplate) {
        await writeClientServicesCustomTemplate(clientFinal, output, httpClient, useOptions, useUnionTypes, indent, postfixServices, serviceTemplate, exportClient, clientName);
    }
    if (clientTemplate) {
        await writeClientClassCustomTemplate(clientFinal, output, httpClient, useOptions, useUnionTypes, indent, postfixServices, clientTemplate, clientName);
    }
    if (indexTemplate) {
        await writeClientIndexCustomTemplate(clientFinal, output, httpClient, useOptions, useUnionTypes, indent, postfixServices, postfixModels, indexTemplate, exportCore, exportServices, exportModels, exportSchemas, exportClient, clientName);
    }
};
var index = {
